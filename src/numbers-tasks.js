...
function getAverage(value1, value2) {
  ...
  if (max === Number.MAX_VALUE) {
    return min / 2 + Number.MAX_VALUE / 2;
  }
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const points = (x2 - x1) ** 2 + (y2 - y1) ** 2;
  const sqrt = Math.sqrt(points);
  return sqrt;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angelInRadius = Math.acos(dotProd);
  return angelInRadius;
}
...
function isPowerOfTwo(num) {
  ...
  const bitAmount = num
    .toString(2)
    .split('')
    .reduce((acc, intg) => acc + Number(intg), 0);
  if (bitAmount === 1) {
    res = true;
  }
  return res;
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = [x1, x2, x3].reduce((acc, curr) => acc + curr, 0);
